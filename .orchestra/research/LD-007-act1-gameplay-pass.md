...
## Дополнение ACT-I-003 — «Край гоблинов» (ветка `content/ACT-I-003-goblin-frontier`)

После пользовательского ACCEPT ветки LD-007 Act I расширен до 18 stage'ов с тремя новыми видами
(`goblin-grunt` — щитоносец, `goblin-matron` — support/heal, `goblin-drunkard` — шатается при
попадании) и четырьмя data-driven механиками поверх COMBAT-001 (без изменений core/solver/win):

- `ability.kind: 'heal'` — раз в N ходов лечит самого раненого союзника на `amount`;
- `ability.kind: 'shift'` + `trigger: 'hit'` — каждое не убивающее попадание сбивает моба на соседнюю сторону;
- `EnemyDef.reward {heal, rotate}` — награда за убийство (heal с cap по max HP, rotate в shared pool);
- `EnemyDef.expiresAfter` (+ `mandatory:false`) — временная цель, уходит через N ходов;
- `EncounterDef.winHeal` — отдых после боя (claim в `RunState.advance`, как `winRotateReward`).

Цепочка (профиль short везде, кроме 10; 0R = минимальный урон без Rotate; naive = урон игрока-без-плана):

| # | stage | board | суть | 0R | naive |
|---|---|---|---|---|---|
| 1–4 | патруль, ученик, камнемёт, разведчик | 6/6/7/7 | как в LD-007 | 0 | 2–3 |
| 5 | Пьяный дозор | 6x6 s27 | пьяница E: каждый хит сбивает его на W и обратно → чередуй стороны | 0 | 2.4 |
| 6 | Волчья тропа | 7x7 s108 | 2 волка (IN2/IN3, d1) + мародёр N: уйдёт через 5 ходов, за него +2 HP | 0 (−2+2) | 2.9 |
| 7 | Засада с трёх сторон | 7x7 s50 | как в LD-007 | 0 | 3.9 |
| 8 | Матрона | 7x7 s71 | лечит грунта на 2 каждые 2 хода — пока жива, грунта не пробить: сначала она | 0 | 3.2 |
| 9 | Семейная ссора | 7x7 s118 | матрона лечит, муж-пьяница убегает от твоих стрел | 0 | 4.5 |
| 10 | Капитан стражи | 7x7 mixed s54 | теперь species goblin-grunt | 0 | 3.0 |
| 11 | Обоз | 7x7 s67 | носильщик N уйдёт через 6 ходов (+3 HP), щитоносец E, налётчик W; после боя +2 HP | 0 | 3.1 |
| 12 | Паучий подвал | 7x7 s53 | два быстрых паучка (IN2/IN3 d1) + brute W 4hp d3 | 0 | 1.8 |
| 13 | Шаманский круг | 8x8 s44 | два кастера N/W (interrupt оба) + пьяница E, которому некуда бежать, пока стороны заняты | 0 | 5.2 |
| 14 | Стена щитов | 8x8 s9 | два щитоносца + матрона: **единственный attrition-stage, 2 неизбежных урона**, компенсируются winHeal 2 | 2 | 4.0 |
| 15 | Ритуальные ворота | 8x8 s65 | как в LD-007 | 0 | 4.1 |
| 16 | Ночная охота | 8x8 s75 | волк E + альфа W, который каждые 2 хода уходит на N; мародёр на 5 ходов | 0 | 2.1 |
| 17 | Королевская стража | 8x8 s76 | щит + пьяница + матрона: убьёшь матрону — пьяница начинает бегать (N освободилась) | 0 | 6.8 |
| 18 | Goblin King | 8x8 s73 | как в LD-007 | 0 | 4.5 |

Сцены: каждому stage назначена своя арена из библиотеки (podium-template калибровка; точная подгонка —
в Campaign Editor). Новые виды: позы в `assets/enemies/goblin-{grunt,matron,drunkard}/`, pivot/scale
в `creature-poses.json` — стартовые, тюнить в Pose Editor.

USER DECISIONS (ACT-I-003): (1) `reward.rotate` реализован, но в контенте не используется — трогает
Rotate economy; (2) `winHeal` на 11 и 14 — первый шаг к экономике HP, число условное; (3) «Стена щитов»
как осознанный attrition-stage; (4) длина Act I — 18 боёв.
