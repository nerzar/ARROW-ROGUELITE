...
createServer(async (req, res) => {
  ...
  // hudOffset?: {x,y}, shadowOffset?: {x,y} }. For each assigned
  ...
  if (req.method === 'POST' && url.pathname === '/api/creature-poses/save') {
    ...
    req.on('end', async () => {
      try {
        const { species, sourceFolder, poses, pivot, scale, hudOffset, shadowOffset } = JSON.parse(body)
        ...
        manifest[species] = { poses: savedPoses, sourceFiles: poses, pivot: xyOrNull(pivot), scale: typeof scale === 'number' ? scale : 1, hudOffset: xyOrNull(hudOffset), shadowOffset: xyOrNull(shadowOffset) }
