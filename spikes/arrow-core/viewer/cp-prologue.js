// EXP-010 combat-pressure prologue viewer: encounters 1-5 chained through a RunState so player HP
// persists across boards. Rendering is adapted from EXP-009's prologue.js; new here is the RunState
// wrapper, the HP bar, the ATTACK IN N readout next to the target, and damage feedback (screen
// flash + message). Rules live entirely in EncounterState/RunState (src/), not in this file.
import {
  analyzeSeed,
  DIR_NAMES,
  DX,
...
const ui = {
  steps: $('steps'), stepTitle: $('stepTitle'), pick: $('pick'), hpText: $('hpText'), hpPips: $('hpPips'),
...
async function loadRunConfig() {
  try {
    return await fetchJson('../encounters/cp-run-config.json')
  } catch (e) {
    console.warn(e)
    return { playerMaxHp: 10 }
  }
}
...
async function loadCandidates() {
  ...
  for (const step of STEPS) {
    const list = []
    const prov = await fetchJson(step.file)
...
// ---------------------------------------------------------------------------------------------
// Run-level state: RunState + which board candidate each step currently uses.
...
function buildStep(stepIndex, raw) {
  const parsed = encounterFromJson(raw)
  return { id: STEPS[stepIndex].key, level: parsed.level, def: parsed.file.encounter, board: parsed.file.board }
}

async function startPrologue(runConfig) {
  const steps = candidates.map((_, i) => {
    const cand = candidates[i][chosen[i]] ?? candidates[i][0]
    return buildStep(i, cand.raw)
  })
  run = new RunState(runConfig, steps)
  loadCurrentBoard()
}
...
function loadCurrentBoard() {
  ...
  const cand = candidates[run.stepIndex][chosen[run.stepIndex]] ?? candidates[run.stepIndex][0]
  ui.why.textContent = cand.why ? cand.why.join('\n') : (def.notes ?? '(see encounter notes)')
}

function fillPickForCurrentStep() {
  ui.pick.innerHTML = ''
  const list = candidates[run.stepIndex]
  for (const c of list) ui.pick.append(new Option(c.label, c.key))
  ui.pick.disabled = list.length <= 1
  ui.pick.selectedIndex = Math.min(chosen[run.stepIndex], list.length - 1)
}
...
function renderStepsBar() {
  ...
  STEPS.forEach((_step, i) => {
    ...
    if (i < run.stepIndex) span.className = 'done'
    else if (i === run.stepIndex) span.className = 'now'
    ui.steps.append(span)
  })
  ui.stepTitle.textContent = STEPS[run.stepIndex].title
}
...
function rotate(turn) {
  ...
  log.push(`${formatAction({ kind: 'rotate', turn })}  → ${s.rotation * 90}°` + (s.playerHp !== hpBefore ? `  ENEMY -${hpBefore - s.playerHp}` : ''))
  afterAction()
...
function scheduleWin() {
  const last = run.isLastStep
  ui.canvas.style.opacity = '0.18'
  overlayTimer = setTimeout(() => showOverlay(last ? 'won' : 'next'), 550)
}
function scheduleGameOver() {
  ui.canvas.style.opacity = '0.4'
  overlayTimer = setTimeout(() => showOverlay('dead'), 550)
}
...
function showOverlay(kind) {
  ...
  if (kind === 'dead') {
    ...
    body.textContent = `HP игрока закончилось на "${STEPS[run.stepIndex].title}".`
    next.textContent = 'restart prologue'
    next.onclick = () => { hideOverlay(); restartRun() }
  } else if (kind === 'won') {
    title.textContent = 'Пролог пройден'
    body.textContent = `Mini-boss повержен. HP на финише: ${run.encounter.playerHp}/${run.maxHp}.`
    next.textContent = 'restart prologue'
    next.onclick = () => { hideOverlay(); restartRun() }
  } else {
    title.textContent = `Encounter ${run.stepIndex + 1} пройден`
    body.textContent = `HP: ${run.encounter.playerHp}/${run.maxHp} → ${STEPS[run.stepIndex + 1].title}`
    next.textContent = 'next →'
    next.onclick = () => { hideOverlay(); run.advance(); loadCurrentBoard() }
    overlayTimer = setTimeout(() => { hideOverlay(); run.advance(); loadCurrentBoard() }, 1600)
  }
}
...
function render(now = performance.now()) {
  ...
  for (const a of level.arrows) drawArrow(a, col, s)
...
function renderPanel() {
  ...
  const alive = s.aliveByArenaDir()
  const free = s.aliveByArenaDir(true)
  const state = s.playerDead ? 'ПОРАЖЕНИЕ (HP закончилось)' : s.won ? 'ЦЕЛЬ ВЫПОЛНЕНА' : 'бой'
  const lines = [
    `${def.title ?? def.id}`,
    `Encounter ${run.stepIndex + 1}/${STEPS.length}`,
...
ui.restartStep.onclick = restartStep
ui.restartAll.onclick = restartRun
ui.liveCheck.onchange = afterAction
ui.pick.onchange = () => {
  // RunState.steps is fixed at construction, so swapping a board mid-run rebuilds the run. This is a
  // debug convenience (try a different E3/E4 candidate) and resets to full HP at the current step --
  // a real playthrough never swaps boards mid-run, so RunState has no API to preserve HP through this.
  chosen[run.stepIndex] = ui.pick.selectedIndex
  const idx = run.stepIndex
  const steps = candidates.map((_, i) => buildStep(i, (candidates[i][chosen[i]] ?? candidates[i][0]).raw))
  run = new RunState(run.config, steps)
  run.debugJumpTo(idx)
  loadCurrentBoard()
}
...
await startPrologue(runConfig)

// Debug hook for automated play checks.
window.cpDebug = {
  run: () => run,
  state: () => run.encounter,
  pointOf(id) {
