...
// BUILD-034: the three warm painters that shipped with the BUILD-032 demo, folded into the same
// registry so every variant the user has actually seen is reachable from one selector.
// `warm-bevel` is BUILD-033's accepted default.
const WARM = {
...
MATERIALS.push(
  {
    id: 'warm-bevel', name: 'Тёплый бевел', vibe: 'BUILD-032 · принятый дефолт',
    animated: false,
    paint(ctx, path, bb, geo) {
      edge(ctx, path, Math.max(3, geo.cell * 0.10), WARM.edge)
      ctx.fillStyle = vgrad(ctx, bb, [[0, WARM.hot], [0.55, WARM.base], [1, WARM.deep]])
      ctx.fill(path)
      clipped(ctx, path, () => {
        ctx.globalAlpha *= 0.8
        edge(ctx, path, Math.max(2, geo.cell * 0.07), WARM.light)
      })
    },
  },
  {
    id: 'warm-solid', name: 'Тёплый плоский', vibe: 'BUILD-032 · плоская заливка',
    animated: false,
    paint(ctx, path) {
  ...
  {
    id: 'warm-magic', name: 'Тёплая магия', vibe: 'BUILD-032 · свечение + блик',
    animated: true,
    paint(ctx, path, bb, geo, now, seed) {
      const t = now / 1000
      glowBody(ctx, path, '#c97a1e', WARM.glow, geo.cell * (0.7 + 0.25 * Math.sin(t * 3 + seed)))
      clipped(ctx, path, () => {
        sheen(ctx, bb, geo.cell, t, 0.35, [[0, 'rgba(255,220,150,0)'], [0.5, 'rgba(255,230,170,0.75)'], [1, 'rgba(255,220,150,0)']])
        edge(ctx, path, Math.max(1.5, geo.cell * 0.045), 'rgba(255,214,140,0.9)')
      })
    },
  },
)
